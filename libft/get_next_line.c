...
static char	*slice_buffer(char **buffer, int line_len)
{
	char	*new_buffer;
	int		buffer_len;
...
static char	*extract_line_from_buffer(char **buffer, char *newline_ptr)
{
	char	*line;
	int		line_len;
...
static char	*read_from_fd(char **buffer, int fd)
{
	char	*read_buffer;
	char	*newline_ptr;
	int		bytes;
