...
int	print_string(char *str, t_ft_printf_flags *flags)
{
	...
	if (flags->minus)
	{
		bytes_written = print_precised_string(flags->fd, str, flags->precision);
		if (bytes_written == -1)
			return (-1);
		spaces_written = print_width(flags->fd, flags->width - bytes_written, ' ');
		if (spaces_written == -1)
			return (-1);
	}
	else
	{
		spaces_written = print_width(flags->fd, calculate_width(str, flags), ' ');
		if (spaces_written == -1)
			return (-1);
		bytes_written = print_precised_string(flags->fd, str, flags->precision);
		if (bytes_written == -1)
...
static int	print_hex_str(char *n_str, t_ft_printf_flags *flags, char sp, unsigned int n)
{
	...
	if (flags->minus)
	{
		if (flags->hashtag && print_hex_prefix(flags->fd, n, sp) == -1)
			return (-1);
		byt_wri = print_precised_number(n_str, flags, n);
		if (byt_wri == -1)
			return (-1);
		wdt_wri = print_width(flags->fd, calc_total_width(n_str, flags, byt_wri), ' ');
		if (wdt_wri == -1)
			return (-1);
	...
	if ((!flags->zero && flags->width)
		...
		wdt_wri = print_width(flags->fd, calc_total_width(n_str, flags, 0), ' ');
	if (wdt_wri == -1 || (flags->hashtag && print_hex_prefix(flags->fd, n, sp) == -1))
		return (-1);
	byt_wri = print_precised_number(n_str, flags, n);
	if (byt_wri == -1)
		return (-1);
