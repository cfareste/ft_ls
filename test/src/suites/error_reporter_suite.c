...
static void should_print_the_read_directory_error_with_the_specified_directory_path(void)
{
    ...
    report_reading_directory_error(directory_path);
...
static void should_only_print_the_access_file_error_if_a_NULL_file_path_is_specified(void)
{
    errno = ENOTDIR;

    report_access_file_error(NULL);

    CU_ASSERT(verify_that_the_error_printed_is("%s: %s\n", ACCESS_FILE_ERROR_PREFIX, strerror(errno)));
}

static void should_only_print_the_access_file_error_if_an_empty_file_path_is_specified(void)
{
    errno = ENOTDIR;

    report_access_file_error("");

    CU_ASSERT(verify_that_the_error_printed_is("%s: %s\n", ACCESS_FILE_ERROR_PREFIX, strerror(errno)));
}

static void should_print_the_access_file_error_with_the_specified_file_path(void)
{
    errno = ENOENT;
    const char *directory_path = "not_existing_file";

    report_access_file_error(directory_path);
